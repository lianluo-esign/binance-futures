...
## 一、WebSocket连接管理器开发 只订阅depth和trades 每一个交易所的ws连接启动一个独立的线程来管理 ✅ 已完成
...
- ✅ 实现交易所类型枚举和配置管理
- ✅ 实现线程化多交易所管理器(ThreadedMultiExchangeManager)
- ✅ 实现独立线程管理和数据标准化框架
...
### 1.3 针对每个交易所的depth数据和trades的数据做标准数据格式化成统一格式，在每个交易所ws连接线程内标准化格式 再投放到eventbus
✅ 已完成基础架构和数据标准化框架
- ✅ 创建StandardizedMarketData结构体
- ✅ 在ExchangeWebSocketManager trait中添加数据标准化方法
- ✅ 实现ThreadedMultiExchangeManager支持独立线程管理
- ✅ 实现数据批量处理和事件总线集成
- ✅ 为各交易所实现具体的消息类型判断方法（已完成）

标准数据为: 
trade: 
{
  "exchange": "okx|bitget|bitfinex...",
  "data_type": "trade",
  "data": Object,  // 不同交易所的message raw_data  
  "timestamp": u64
}
...
## 二、数据格式转换Handler 需要从lock free buffer eventbus当中消费出数据来做后续动作

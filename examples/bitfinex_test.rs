...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("开始测试Bitfinex WebSocket连接...");
    
    // 创建Bitfinex WebSocket管理器
    let mut manager = BitfinexWebSocketManager::new();
    
    // 连接到Bitfinex WebSocket
    match manager.connect().await {
        Ok(_) => {
            info!("成功连接到Bitfinex WebSocket");
            ...
            if let Err(e) = manager.disconnect().await {
                ...
                info!("已断开Bitfinex WebSocket连接");
            }
        }
        Err(e) => {
            error!("连接Bitfinex WebSocket失败: {}", e);
        }
    }
    
    info!("Bitfinex WebSocket测试完成");
    Ok(())
} 